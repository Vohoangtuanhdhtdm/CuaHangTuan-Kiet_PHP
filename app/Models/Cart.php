...
class Cart {
    ...
    // Lấy ID giỏ hàng hiện tại (hoặc tạo mới nếu chưa có)
    public function getCurrentCartId() {
        $userId = $_SESSION['user_id'] ?? null;
        $sessionId = session_id();

        if ($userId) {
            // Khách đã đăng nhập -> Tìm theo user_id
            $sql = "SELECT id FROM carts WHERE user_id = :user_id LIMIT 1";
            $stmt = $this->db->prepare($sql);
            $stmt->execute(['user_id' => $userId]);
        } else {
            // Khách vãng lai -> Tìm theo session_id
    ...
    public function getCartItems($cartId) {
        // ĐÃ NÂNG CẤP: Bổ sung ci.variant_id vào sau ci.quantity
    ...
    public function addItem($cartId, $productId, $variantId, $quantity) {
        ...
        $stmtCheck->execute([
            ...
            'variant_id2' => $variantId // Hack nhỏ của PDO để so sánh NULL
        ]);
    ...
    public function updateItem($cartItemId, $cartId, $quantity, $variantId) {
        try {
            ...
            // Bắt lỗi trùng lặp Unique Key (Nếu user đổi size/màu sang một phân loại đã có sẵn trong giỏ)
            return false; 
        }
    }
